...
import { apiGet, apiPost, apiDelete, mapConversation, mapMessage, type Conversation, type Message } from "@/lib/api";
...
export default function ConversationPage() {
  ...
  useEffect(() => {
    ...
    const unsubMessage = subscribe(`/topic/conversations/${id}`, (msg: any) => {
    ...
    const unsubEvents = subscribeUser("/queue/events", (event: any) => {
      if (event.type === "read" && event.conversationId === Number(id)) {
        setMessages((prev) =>
          prev.map((m) =>
      ...
      if (event.type === "cleared" && event.conversationId === Number(id)) {
        setMessages([]);
      }
      if (event.type === "deleted" && event.conversationId === Number(id)) {
  ...
  const handleClearChat = async () => {
    if (!id) return;
    try {
      await apiDelete(`/conversations/${id}/messages`);
      setMessages([]);

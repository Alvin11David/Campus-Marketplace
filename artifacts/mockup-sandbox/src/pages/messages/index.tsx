...
export default function MessagesPage() {
  ...
  useEffect(() => {
    const unsub = subscribeUser("/queue/unread", (update: any) => {
      if (update && update.conversationId) {
            ...
            return prev.map((c) =>
              c.id === update.conversationId
                ? {
                    ...
                    last_message_preview: update.lastMessagePreview ?? c.last_message_preview,
                    last_message_at: update.lastMessageAt ?? c.last_message_at,
                  }
                : c
            );
          }
